...
(async function initAutoLogin() {
    ...
    // 2. 注入浮動視窗：首頁、任務頁面、Cookie 過期被跳轉回的 /access 身分證登入頁均顯示
    const isTargetPage = currentUrl === 'https://500.gov.tw/registrant/' ||
        currentUrl === 'https://500.gov.tw/registrant' ||
        currentPath === '/registrant/access' ||
        currentPath.includes('/registrant/member/tasks');

    if (isTargetPage) {
        injectFloatingUI();
    }

    // 3. 身分證號登入頁面：若處於自動登入流程則自動填寫送出
...
async function injectFloatingUI() {
    ...
    settingsPanel.innerHTML = `
        ...
        <div class="al-footer-meta">
            v1.1.2 · Developed by <a href="https://github.com/blues32767/sport115-sweat-autologin/" target="_blank" rel="noopener">blues32767</a> · 2026/09/06
        </div>
    `;
    ...
    // 開關點擊：若已在 /access 頁面則直接重新整理執行填表，若在其他頁則導向登入頁
    toggleBtn.addEventListener('click', (e) => {
        ...
        sessionStorage.setItem('autoLoginFlow', 'true');
        setTimeout(() => {
            if (window.location.pathname === '/registrant/access') {
